...
import { ServiceHistoryRepository } from "./SRHistory.repository";
...
export class SRHistoryService {

  public constructor (private readonly serviceHistoryRepository: ServiceHistoryRepository) {
    this.serviceHistoryRepository = serviceHistoryRepository;
  }

  public async getSPSRHistory (userId: number): Promise<ServiceRequest[] | null> {
  ...
  public async getDatForExport(serviceRequest: ServiceRequest[]): Promise<Object[]>{
    ...
    for (let history in serviceRequest) {
      let user = await this.serviceHistoryRepository.getUserDetailById(serviceRequest[history].UserId);
      exportHistory.push({
  ...
  public async gethistoryForDisplay (serviceRequest: ServiceRequest[]) {
    ...
    for (let sr in serviceRequest) {
      let user = await this.serviceHistoryRepository.getUserDetailById(serviceRequest[sr].UserId);
      let address = await this.serviceHistoryRepository.getServiceAddress(serviceRequest[sr].ServiceRequestId);
