                    ...
                    return [2 /*return*/, this.upcomingService
                            ...
                            if (serviceRequests) {
                                if (serviceRequests.length > 0)
                                    ...
                                    return res.status(404).json({ message: "no upcoming service requests found" });
                            }
                            else
                                return res.status(404).json({ message: "no upcoming service requests found" });
                        })
                    ...
                    if (req.params.SRId)
                        return [2 /*return*/, this.upcomingService
                                ...
                                if (serviceRequest) {
                                    if (serviceRequest.ServiceProviderId === req.body.userId)
                                        return _this.upcomingService
                                            ...
                                            if (updatedrequest[0] === 1)
                                                ...
                                                return res.status(422).json({ message: "error in cancelling service request" });
                                        })
                                            .catch(function (err) { return res.status(500).json({ error: err }); });
                                    else
                                    ...
                                    return res.status(404).json({ message: "service request detail not found" });
                            })];
                    else
                        return [2 /*return*/, res.status(400).json({ message: "service request id not found" })];
                }
                else
                    return [2 /*return*/, res.status(401).json({ message: "unauthorised user" })];
                return [2 /*return*/];
                                            ...
                                            if (serviceRequest)
                                                return _this.upcomingService
                                                    ...
                                                    if (updatedrequest[0] === 1)
                                                        ...
                                                        return res.status(422).json({ message: "error in updating service request" });
                                                })
                                                    .catch(function (err) { return res.status(500).json({ error: err }); });
                                            else
                                                return res.status(400).json({ message: "You can not complete service request before end time" });
                                        })
                                            .catch(function (err) { return res.status(500).json({ error: err }); });
                                    }
                                    else
