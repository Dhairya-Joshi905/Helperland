...
var BookService = /** @class */ (function () {
    ...
    BookService.prototype.createData = function (userEmail, srId) {
        var data = {
            ...
            html: "<h1>A service request ".concat(srId, " has been directly assigned to you.</h1>")
        };
        return data;
    };
    BookService.prototype.createDataForAll = function (userEmail) {
        var data = {
            ...
            html: "<h1>New service is available in your area login and accept before anyone accept it.</h1>"
        };
        return data;
    };
    BookService.prototype.createToken = function (userEmail, postalCode) {
        // This is 3 hours maybe
        var token = jsonwebtoken_1.default.sign({ userEmail: userEmail, postalCode: postalCode }, process.env.SECRET_KEY, { expiresIn: "5h" });
        return token;
    };
        ...
        if (body.HasPets === true)
            for (var count in user)
                if (user[count].WorksWithPets === true)
                    ...
                    for (var count_1 in user)
                        Email.push(user[count_1].Email);
        return Email;
    };
