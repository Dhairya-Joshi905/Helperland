    ...
    function DashboardController(dashboardService) {
                ...
                if (greaterThanPresentTime) {
                    if (req.body.userTypeId === 4)
                        return [2 /*return*/, this.dashboardService
                                ...
                                .then(function (SR) {
                                if (SR) {
                                    req.body.totalHour = SR.ExtraHours + SR.ServiceHours;
                                    if (SR.UserId === req.body.userId) {
                                        if (SR.ServiceProviderId) {
                                            req.body.spId = SR.ServiceProviderId;
                                            return _this.dashboardService
                                                .getSPDashboard(SR.ServiceProviderId)
                                                ...
                                                return __generator(this, function (_b) {
                                                    switch (_b.label) {
                                                        ...
                                                        case 1:
                                                            ...
                                                            if (matched)
                                                                return [2 /*return*/, res.status(200).json({ message: "Service provider is busy with another service request on " + srDate + " from " + startTime + " to " + endTime + ". Either choose another date or schedule on a different time." })];
                                                            else
                                                                next();
                                                            return [3 /*break*/, 3];
                                                        case 2:
        ...
        this.rescheduleByCustomer = function (req, res) { return __awaiter(_this, void 0, void 0, function () {
            ...
            return __generator(this, function (_a) {
                ...
                if (req.params.SRId)
                    return [2 /*return*/, this.dashboardService
                            .rescheduleSR(new Date(date), req.body.time, parseInt(req.params.SRId))
                            .then(function (SRArray) {
                            if (SRArray.length > 0) {
                                if (SPId)
                                    return _this.dashboardService
                                            ...
                                            if (serviceRequestArray.length > 0) {
                                                if (serviceRequest.ServiceProviderId)
                                                    return _this.dashboardService
                                                        ...
                                                        .then(function (serviceProvider) {
                                                        if (serviceProvider === null || serviceProvider === void 0 ? void 0 : serviceProvider.Email) {
                                                            var data = _this.dashboardService.cancelSRData(serviceProvider.Email, SRId);
                                                            mg.messages().send(data, function (err, body) {
                                                                if (err)
                                                                    return res.json({ error: err.message });
                                                            });
